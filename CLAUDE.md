...
- **Country-flag override: LT is the *only* exception**, and even then **not on bingo**. `lib/fluent-emoji.ts` carries a `url` field for `🇱🇹` pointing at our Supabase-hosted hand-rendered Fluent-style PNG, so surfaces where Lithuania is part of the brand identity (presence-bar idle eyebrow, home bet-chips marquee) render the flag in 3D. Do NOT add more country-flag URL overrides — every other country falls back to the native OS glyph, and the bingo rule still applies (LT included).
