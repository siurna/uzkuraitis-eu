...
export default async function AdminRoomDetailPage({
  ...
  // Aggregate reactions per country (sum + breakdown).
  ...
  // Score totals for this room (cheap recompute, no view).
        ...
        {scoreRows.length === 0 ? (
          ...
          <ol className="grid grid-cols-1 md:grid-cols-2 gap-1 text-sm">
            {scoreRows.map((row, i) => {
              ...
              const reactions = reactionByCountry.get(row.countryCode);
              return (
                <li
                  key={row.countryCode}
                  ...
                  {reactions && (
                    ...
                    >
                      {reactions.emojis
                        ...
                        .map((r) => r.emoji)
                        .join("")}
                      <span className="ml-1 tabular-nums">{reactions.total}</span>
                    </span>
                  )}
      ...
      {reactionRows.length > 0 && (
        <section className="glass-card rounded-xl p-5">
          ...
          <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2 text-sm">
            {Array.from(reactionByCountry.entries()).map(([code, agg]) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="flex-1 truncate">{c?.name ?? code}</span>
