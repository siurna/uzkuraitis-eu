...
// Home widget: the chat moments that blew up. Each row is who said it,
// the country that was on stage at the time, the message, and the
// reaction tally. Tap any → jump to the chat tab. Self-hides if empty.
    ...
    <div className="container mx-auto max-w-3xl px-4 flex flex-col gap-3">
      ...
      <ul className="flex flex-col gap-2">
        {items.map((h) => {
          ...
          const text =
            h.kind === "text" ? (h.body ?? "") : h.kind === "bingo_strike" ? "🎯 Bingo!" : h.gifUrl ? "GIF" : (h.body ?? "");
          return (
            <li key={h.id}>
              <button
                type="button"
                onClick={() => setTab("chat")}
                className="w-full text-left flex items-center gap-3 rounded-2xl bg-white/[0.04] ring-1 ring-white/8
                           hover:bg-white/[0.07] transition transform-gpu duration-150 active:scale-[0.99] px-3 py-2.5"
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-sm text-white/90 leading-snug truncate">{text}</p>
                </div>
